#include "gaincoinsifnobuycommand.h"
#include "player.h"
#include "card.h"
#include "gamestate.h"
#include "gamecontroller.h"

GainCoinsIfNoBuyCommand::GainCoinsIfNoBuyCommand(Player* sourcePlayer, QObject* parent,QList<Card*> cards,Player* activePlayer)
    : GameCommand(CommandType::GainCoinsIfNoBuy, sourcePlayer,parent,cards,activePlayer){}

void GainCoinsIfNoBuyCommand::execute(GameState* state, GameController* controller) {
    //计算有多少卡牌
    ...
    if(coinsSum==0)
        ...
        log+=QString("因为本回合没建设建筑，获得 %1 金币。").arg(coinsSum);
