...
// 获得选项
QVariantList Card::getChoices(Player* owner, Player* activePlayer, int rollSum, Game* game) const {
    Q_UNUSED(owner); Q_UNUSED(activePlayer); Q_UNUSED(rollSum); Q_UNUSED(game);
    return {}; // 默认返回空列表，表示该卡牌在掷骰子激活后不需要玩家选择
}

// 自动推荐选项
QVariant Card::autoChoices(Player* owner, Player* activePlayer, int rollSum, Game* game,QVariantList choices) const {
