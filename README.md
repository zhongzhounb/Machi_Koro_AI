...
- 【搬家公司】：这个可以用自己的近期收益，减去自己牌中的近期收益最小值（可能为负值，比如后期的拆迁公司，这样会让系统自动在后期将拆迁公司给获胜玩家）。因为可以把自己送出去，为了防止无限递归，需要设个初始值，我们假设他初始价值就是购买时的价值。
  ...
  MovingCompanyEx(C_x,P_y,z)=\begin{cases}
      ...
      \sum_{i=L_x}^{R_x}(Value(C_X)-Coins(C_x))\times Prob(P_y,D_i) & \text{otherwise }  \\
  \end{cases}
  $$
  其中 $K$ 为当前玩家拥有的牌的数量。
...
- 【科技公司】：非常特殊的卡牌！他的收益是场上唯一一个不能立即体现的卡牌，需要”前期买，后期收益大“，如果考虑近期收益的话确实是0，但是设置值的话违背了”近期收益“的概念，所以**该卡只有未来收益，会在之后的未来贡献做特判**。
...
#### 未来贡献：某张卡在未来很远的一轮对自己的期望收益

**主要目的：引导AI从买6-卡到7+卡，并开火车站**

在游戏前期，如果所有人都没有火车站，会导致某些7+的卡近期贡献一直为0，所以一直买6-的卡，恶性循环，最后把6-卡买光了都不会开火车站。

所以需要计算未来贡献来引导AI尝试为未来的卡布局，但是未来的概率不好计算，那就取个极限状态，来修改近期贡献的$Prob(P_i,D_j)$——

**极限状态**：假设现在场上有了所有卡牌，所有人地标只差机场，所有人都只差一步获胜，所有人都只管自己收益最大：
...
| ----------------- | ------ | ---- | ------ | -------- | -------- | ---- | ------ | ------ | ------ | -------- | -------- | ---------- |
| 蓝绿卡vs红卡      | 寿司店 | 农场 | 咖啡馆 | 拆迁公司 | 法国餐厅 | 花店 | 出版社 | 税务所 | 税务所 | 科技公司 | 果蔬超市 | 会员俱乐部 |
| 两点数相同        | \      | √    | \      | √        | \        | √    | \      | √      | \      | √        | \        | √          |
| 两点数不同/一点数 | ×      | √    | ×      | ×        | ×        | √    | √      | √      | √      | √        | √        | \          |
...
因为近期贡献和未来贡献都是“某张卡一轮的收益”，所以按照每个玩家都是这个投掷概率作为$Prob(P_i,D_j)$，带入$RecentEx(C_x,P_y,z)$计算得到$FutureEx(C_x,P_y,z)$即可。

注意：由于”只差机场“，【购物中心】对关联卡牌的影响变化和上文提到的【科技中心】的影响变化较大，需要特判卡的$Value(C_x)$：

- 【购物中心】关联卡：$Value(C_x)=Value(C_x)+1$
- 【科技中心】：$Value(C_x)=10$（系统存到10元不再存储）

通过合理设置近期贡献和未来贡献的权重，能让AI买卡策略从6-过度到7+：

1. 没人买7+及开火车站
2. 未来贡献权重持续提升
3. 1号AI发现【果园】总贡献最高，买下，之后未来贡献使【果园】一直很高，1号AI一直在买【果园】（未来贡献引导AI买7+蓝卡）
4. 1号发现自己的最大收益期望在7+，于是开了【火车站】，之后开始抛两个骰子。（7+卡引导AI开火车站）
5. 1号突然发现某些紫卡收益大（因为自己能抛出来了），买下（开火车站引导AI买7+紫卡）
