...
$$
GreenEx(x,y)=\begin{cases}
    \sum_{i=L_x}^{R_x}Value(C_x)\times Prob(P_y,D_x) & \text{if } M=0 \\
...
- 【拆迁公司】：
  $$
  DemolitionCompanyEx(x,y)=GreenEx(x,y)-
  $$
...
$$
BlueEx(x,y)=\sum_{i=1}^{N}\sum_{j=L_x}^{R_x} Value(C_x)\times Prob(P_i,D_j)+\sum_{i=1}^M\sum_{j=L_i}^{R_i}Num(P_y,C_i)\times Value(C_i)\times Prob(P_y,D_j)
$$

特殊蓝卡：

- 【金枪鱼船】：$Value(C_x) $ 取抛两骰子期望值6.5
...
$$
RedEx(x,y)=\sum_{\substack{i=1 \\ i \neq y}}^{N}\sum_{j=L_x}^{R_x} min(Value(C_x),Coins(P_i))\times p(P_i,D_j)
$$

##### 紫卡：

$$
PurpleEx(x,y)=\sum_{\substack{i=1 \\ i \neq y}}^{N}\sum_{j=L_x}^{R_x} min(Value(C_x),Coins(P_i))\times p(P_y,D_j)
$$
...
假设现在场上有了所有卡牌，所有人地标只差机场，所有人的钱在10元上下，所有人都只管自己收益：
...
因为近期贡献和未来贡献都是“某张卡一轮的收益”，所以按照每个玩家都是这个投掷比例，带入计算即可。
...
所以，在临近游戏终局时，尤其是”一局定胜负“时，不在意单个卡牌对自己的影响，而在意是否可以存在抛出收益最大

这个公式也很好算，就是近期贡献的概率设为1，再将点数相同的相加进来就好了。



#### 干扰贡献：某张卡在前一轮对一个其他玩家的单次最大负收益
